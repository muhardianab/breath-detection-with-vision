...
def get_roi(cascade_result, img):
    for (x, y, w, h) in cascade_result:
        roi_gray = img[y:y+h, x:x+w]   #(ycoord-start, ycoord-end)
        return roi_gray
...
path = "/home/agus/BELAJAR/PROJECT/DATA SCIENCE/medial.io-be/venv/lib/python3.6/site-packages/cv2/data/"
noseCascade = cv2.CascadeClassifier(path+'haarcascade_frontalface_default.xml')
...
while(True):
    ...
    # roi
    roi_next = get_roi(nose, next_image)
    
    if(roi_next is not None):
        cv2.imshow('frame_roi', roi_next)

        # Optical Flow - Dense / Farneback
        flow = cv2.calcOpticalFlowFarneback(roi_temp, roi_next, None, 0.5, 3, 15, 3, 5, 1.2, 0)
        mag, ang = cv2.cartToPolar(flow[..., 0], flow[..., 1])
        # ang = np.array(ang).astype(object)
        # mag = np.array(mag).astype(object)
        if(ang.shape == (120, 120) and mag.shape == (120, 120)):
